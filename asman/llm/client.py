...
class LLMClient:
    ...
    async def chat(self, 
        ...
        if self._circuit_open:
            raise RuntimeError("LLM 熔断中，拒绝请求")

        last_exc = None
        for attempt in range(self.max_retries + 1):
            try:
                resp = await self._chat_dispatch(system_prompt, user_prompt, model,
    ...
    async def _chat_dispatch(self, system_prompt, user_prompt, model,
                             temperature, max_tokens, response_format) -> LLMResponse:
        if self.provider == "openai":
    ...
    def _is_retryable(self, exc: Exception) -> bool:
        """可重试错误：超时 / 限流(429) / 服务端(5xx) / 连接错误"""
        name = type(exc).__name__.lower()
        msg = str(exc).lower()
        return any(k in name or k in msg for k in
                   ("timeout", "rate", "429", "503", "500", "connection", "server", "overload"))
    ...
    def get_cost_estimate(self, response: LLMResponse) -> float:
        ...
        按模型名匹配计价表，价格为每 1K token 美元数。
        未匹配到的模型返回 0.0（如本地 Ollama / 未知中转）。
        """
        ...
        for key, (in_price, out_price) in PRICES.items():
            if key in model:
                return (prompt * in_price + completion * out_price) / 1000
        return 0.0
