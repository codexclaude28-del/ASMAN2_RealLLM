"""ASMAN 通用核心模型 —— 地铁隐喻下的运行时对象

与业务无关：乘客/列车/票据/行程/质量评分等抽象，
业务自定义参数统一放在 TaskConfig.params 中。
"""
...
class TaskConfig:
    ...
    业务相关字段（如题材、章节数、目标平台等）不要在这里扩展，
    统一放进 params 字典，由业务方自行读写。
    """
    title: str = ""
    user_input: str = ""
...
class QualityScores:
    ...
    def average(self) -> float:
        return sum([self.coherence, self.creativity, self.consistency,
                    self.grammar, self.engagement]) / 5
